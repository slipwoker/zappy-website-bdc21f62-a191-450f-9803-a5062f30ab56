...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (phoneHeaderBtn) {
    phoneHeaderBtn.addEventListener('click', function() {
      const phoneNumber = '+972508357975';
        ...
        (async function() {
          try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
              ...
              body: JSON.stringify({
                websiteId: 'bdc21f62-a191-450f-9803-a5062f30ab56',
                name: data.name || data.fullName || data.fullname || '',
                email: data.email || '',
                subject: data.subject || 'Contact Form Submission',
                message: data.message || data.comments || data.comment || '',
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "חגי אפרתי משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
